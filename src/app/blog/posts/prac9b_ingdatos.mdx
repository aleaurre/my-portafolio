...
El reporte de clasificación del mejor modelo —un pipeline branched con CatBoost Encoding y HGB— expone un desempeño perfecto en todos los indicadores: precisión, recall y F1 alcanzan el valor máximo para ambas clases. Esto significa que el modelo no solo clasifica correctamente todos los casos, sino que lo hace sin incurrir en falsos positivos ni falsos negativos. En términos prácticos, logra identificar de manera exacta qué propiedades superan la mediana de precio y cuáles no, incluso en presencia de variables categóricas complejas. La simetría entre clases indica que el modelo no depende de un desequilibrio en la distribución de la variable objetivo, mientras que los promedios macro y weighted confirman una estabilidad generalizada del rendimiento. Este tipo de resultado es poco frecuente en datos reales y sugiere que la combinación adecuada de encoding supervisado y modelos basados en gradientes logra capturar patrones muy refinados del dataset Ames Housing.
